...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.video_data_dir = script_args.video_data_dir
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if inputs[0]['data_type'] == 'image':
            input_copy[0]['content'][0]['image'] = os.path.join(self.video_data_dir, inputs[0]['image_path']) 
            #os.getcwd() + "/Video-R1-data" + inputs[0]['path'][1:] 
        elif inputs[0]['data_type'] == 'video':
            input_copy[0]['content'][0]['video'] = os.path.join(self.video_data_dir, inputs[0]['video_path'])   
            #os.getcwd() + "/Video-R1-data" + inputs[0]['path'][1:] 

        video_path = input_copy[0]['content'][0][inputs[0]['data_type']]
        print(f'Video from path: {video_path}, is exists? -> {Path(video_path).exists()}')  
        ...
        try:
            ...
            if inputs[0]['data_type'] == 'image':
                input_copy[0]['content'][0]['image'] = os.path.join(self.video_data_dir, "fixed_data/sample_image.jpg")
            elif inputs[0]['data_type'] == 'video':
                input_copy[0]['content'][0]['video'] = os.path.join(self.video_data_dir, "fixed_data/sample_video.mp4")
